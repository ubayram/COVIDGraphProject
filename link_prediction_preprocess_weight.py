...
def create_dataframes(adj_mat, train_positive_edges, train_negative_edges, test_positive_edges, test_negative_edges):
	
	print('creating test/train dataframes....')

	adj_mat_dense = adj_mat.todense()

	positive_train_edges = pd.DataFrame(train_positive_edges, columns = ['node_1', 'node_2'])  
	for ix, row in  positive_train_edges.iterrows():
		positive_train_edges.loc[ix, 'label'] = adj_mat_dense[(row['node_1'], row['node_2'])]
	...
	for ix, row in  positive_test_edges.iterrows():
		positive_test_edges.loc[ix, 'label'] = adj_mat_dense[(row['node_1'], row['node_2'])]
